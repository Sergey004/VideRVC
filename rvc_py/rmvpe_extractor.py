...
class RMVPE:
    def __init__(self, model_path, device='cpu'):
        ...
        self.is_half = torch.cuda.is_available() and 'cuda' in device
        if device is None:
        ...
        # Продолжение инициализации как в оригинале, без use_jit
        model = E2E(4, 1, (2, 2))
    ...
    def infer(self, audio, sr, thred=0.03):
        ...
        if sr != 16000:
            ...
            audio = librosa.resample(audio_np, orig_sr=sr, target_sr=16000)
            audio = torch.from_numpy(audio)
...
# Удаление блока if __name__ == "__main__":
